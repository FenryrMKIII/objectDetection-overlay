    ...
    for background in backgroundPath.iterdir():
        if background.is_file():
            ...
            for j in range(perBackground):
                ...
                rotate = albumentations.augmentations.transforms.RandomRotate90(p=0.0)
                aug = Compose(
                    [rotate],
                    bbox_params=BboxParams(
                        format="yolo",
                ...
                overliesPerBackground = max(len(overlies), np.random.choice(1))
                ...
                with open(
                    ...
                    for i, bbox in enumerate(bckgWithOverlies["bboxes"]):
                        if args.format == "yolo":
                            yolo = "{:d} {:.4f} {:.4f} {:.4f} {:.4f}\n".format(
                                *[int(bckgWithOverlies["category_id"][i])-1] + bbox
                            )
                            f.write(yolo)
                        elif args.format == "SSD":

                            img_w = bckgWithOverlies['image'].shape[1]
                            img_h = bckgWithOverlies['image'].shape[0]
                            ...
                            SSD = "{:d} {:.4f} {:.4f} {:.4f} {:.4f}\n".format(
                                *[int(bckgWithOverlies["category_id"][i])-1] + [xmin_rel, xmax_rel, ymin_rel, ymax_rel]
                            )
                            f.write(SSD)
                counter+=1
